...
class Player {
    ...
    constructor(age: number, projected_war: number) {
        this.start_age = age;
        this.start_war = projected_war;
    }

    war_projection(age: number, previous_war: number): number {
    ...
    projection(years: number): number[] {
...
class Contract {
    ...
    constructor(years: number, dollars_per_war: number, inflation: number) {
        this.years = years;
...
function contract_projection(initial_years: number, initial_age: number,
                             initial_war: number, initial_cost: number,
...
function format_number(num: number, digits?: number) {
    if (!digits) {
        digits = 1;
    }
...
function table_make_row(fields: any[]) {
...
function calculate_contract(ev) {
    let years_el = <HTMLInputElement>document.getElementById('years');
    let age_el = <HTMLInputElement>document.getElementById('age');
    let war_el = <HTMLInputElement>document.getElementById('war');
    let cost_el = <HTMLInputElement>document.getElementById('cost');
    let inflation_el = <HTMLInputElement>document.getElementById('inflation');
...
function clear_children(element: HTMLElement) {
